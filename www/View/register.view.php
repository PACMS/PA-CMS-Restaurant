<header>
    ...
    window.addEventListener("load", function() {
        ...
        var errorsArray = <?php echo json_encode($errors ?? null); ?>;

        if (errorsArray !== null) {
            console.log("on a des errors");
            for (let i = 0; i < errorsArray.length; i++) {
                const element = errorsArray[i];
                console.log("avant", element);
                $("div.container-register h1").after(`<p style="color: red; margin: 5px 0;">${element}</p>`);
                console.log("après", element);
            }
        } else {
            console.log("pas d'errors");
        }
